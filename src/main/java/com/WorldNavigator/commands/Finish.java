...
public class Finish implements ICommand {
  ...
  public String execute(PlayerInfo player, MapInfo map, List<String> splitCommand) {
    checkArguments(player, map, splitCommand, "Finish");
    ...
    if (player.getCurrentState().getName().equals("roomLevel")) {
      throw new IllegalStateException(
          "player should not be able to execute finish command in roomLevel.");
    }
    player.popState();
    return "You are now looking at the " + player.getCurrentState().getDisplayName() + "!";
  }
}
